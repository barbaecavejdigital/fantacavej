    ...
    return (
        <div className="dark:text-slate-300">
            <input
                ...
                className="w-full px-4 py-3 bg-slate-100 dark:bg-slate-800 border-transparent rounded-xl shadow-sm focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent transition mb-4"
            />
            
            <div className="space-y-3 max-h-80 overflow-y-auto pr-2">
                {isLoading ? <p className="text-center p-4">Caricamento...</p> : filteredActions.map(action => (
                    <div key={action.id} className="flex justify-between items-center p-4 rounded-lg border dark:border-slate-700 bg-white dark:bg-slate-800/50 transition-shadow hover:shadow-md">
                        <div>
                            <p className="font-bold">{action.name}</p>
                            <p className="text-sm text-slate-500 dark:text-slate-400">{action.description || 'Nessuna descrizione'}</p>
                        </div>
        ...
        <div className="dark:text-slate-300">
            <div className="space-y-3 max-h-80 overflow-y-auto pr-2">
                {isLoading ? <p className="text-center p-4">Caricamento...</p> : prizes.map(prize => {
                    ...
                    return (
                        <div key={prize.id} className={`flex justify-between items-center p-4 rounded-lg border dark:border-slate-700 ${canRedeem ? 'bg-white dark:bg-slate-800/50' : 'bg-slate-50 dark:bg-slate-800/20 opacity-60'}`}>
                            <div>
                                <p className="font-bold">{prize.name}</p>
                                <p className="text-sm text-slate-500 dark:text-slate-400">{prize.pointsRequired} punti</p>
                            </div>
...
const HistoryModal: React.FC<{ user: User, onClose: () => void, onUpdate: (user: User, message: string) => void }> = ({ user, onClose, onUpdate }) => {
    ...
    const [activeTab, setActiveTab] = useState<'assigned' | 'redeemed'>('assigned');
    ...
    const renderTable = (txs: PointTransaction[]) => {
        if (txs.length === 0) return <p className="text-center text-slate-500 py-8">Nessuna transazione in questa categoria.</p>;
        return (
            <table className="w-full text-sm text-left table-fixed">
                ...
                <tbody className="dark:text-slate-300">
                    {txs.map(tx => {
                        ...
                        return (
                            <tr key={tx.id} className={`border-b dark:border-slate-700 last:border-b-0 ${tx.isReversed ? 'bg-slate-50 dark:bg-slate-800/30 text-slate-400 dark:text-slate-500' : 'bg-white dark:bg-slate-800/80'}`}>
                                ...
                                <td className={`p-3 break-words text-slate-500 dark:text-slate-400 ${tx.isReversed ? 'line-through' : ''}`}>{description}</td>
                                ...
                                <td className="p-3 text-center">
                                    {!tx.isReversed && (tx.type === 'assignment' || tx.type === 'redemption') && (
                                        <button onClick={() => setTxToReverse(tx)} className="p-1 rounded-full text-slate-500 hover:text-purple-600 hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors" title="Storna Transazione" disabled={isLoading}>
    ...
    return (
        <div className="dark:text-slate-300">
            <div className="border-b border-slate-200 dark:border-slate-700 mb-4">
                <nav className="-mb-px flex space-x-6"><button onClick={() => setActiveTab('assigned')} className={`${activeTab === 'assigned' ? 'border-purple-500 text-purple-600 dark:text-purple-400' : 'border-transparent text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200'} whitespace-nowrap py-3 px-1 border-b-2 font-medium text-sm`}>Punti Assegnati</button><button onClick={() => setActiveTab('redeemed')} className={`${activeTab === 'redeemed' ? 'border-purple-500 text-purple-600 dark:text-purple-400' : 'border-transparent text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200'} whitespace-nowrap py-3 px-1 border-b-2 font-medium text-sm`}>Premi Riscattati</button></nav>
            </div>
            <div className="max-h-[60vh] overflow-y-auto border-t border-slate-200 dark:border-slate-700">{isLoading ? <p className="text-center p-8">Caricamento...</p> : (activeTab === 'assigned' ? renderTable(assignedTransactions) : renderTable(redeemedTransactions))}</div>
            {txToReverse && (<Modal title="Conferma Storno" onClose={() => setTxToReverse(null)}><div className="dark:text-slate-300"><p className="mb-4">Sei sicuro di voler stornare la transazione <span className="font-bold">"{txToReverse.description}"</span>?<br/>Verrà creata una transazione correttiva.</p><div className="flex justify-end gap-3 mt-6"><Button onClick={() => setTxToReverse(null)} variant="secondary" disabled={isLoading}>Annulla</Button><Button onClick={handleConfirmReverse} variant="danger" disabled={isLoading}>Conferma Storno</Button></div></div></Modal>)}
        </div>
    )
}
...
const SortableHeader: React.FC<{columnKey: string; title: string; sortConfig: { key: string; direction: 'ascending' | 'descending' }; requestSort: (key: string) => void;}> = ({ columnKey, title, sortConfig, requestSort }) => {
    ...
    return (<th className="p-4 font-semibold text-sm sticky top-0 bg-slate-100 dark:bg-slate-800 z-10"><button onClick={() => requestSort(columnKey)} className="flex items-center gap-1.5 text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white">{title}{isSorted && <span className="text-xs text-purple-500">{directionIcon}</span>}</button></th>);
};
    ...
    return (
        <Card className="h-full flex flex-col">
            <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4 mb-6 shrink-0">
                <h2 className="text-2xl font-bold text-slate-800 dark:text-white">Elenco Clienti</h2>
                <div className="flex items-center gap-4">
                    <input type="search" placeholder="Cerca cliente..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full sm:w-64 px-4 py-2.5 bg-slate-100 dark:bg-slate-800 border-transparent rounded-xl shadow-sm focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent transition" />
                    <Button onClick={handleCreateCustomer}>Nuovo Cliente</Button>
                </div>
            </div>

            <div className="flex-grow overflow-y-auto min-h-0 -mx-8">
                {isLoading ? (<p className="text-center p-8 text-slate-500">Caricamento clienti...</p>) : (
                <table className="w-full text-left">
                    <thead className="dark:text-slate-300">
                        <tr className="bg-slate-100 dark:bg-slate-800/50">
                            ...
                            <th className="p-4 font-semibold text-sm text-right sticky top-0 bg-slate-100 dark:bg-slate-800 z-10">Azioni</th>
                        </tr>
                    </thead>
                    <tbody className="dark:text-slate-300">
                        {sortedAndFilteredCustomers.map(customer => (
                            <tr key={customer.id} className="border-b dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800/50 hover:shadow-sm hover:-translate-y-px transition-all duration-200">
                                ...
                                <td className="p-4 font-bold text-purple-600 dark:text-purple-400">{customer.points}</td>
                                <td className="p-4 text-sm text-slate-500 dark:text-slate-400 whitespace-nowrap">{new Date(customer.creationDate).toLocaleDateString('it-IT')}</td>
                                <td className="p-4 text-right"><div className="flex justify-end items-center gap-2">
                                    <Button size="sm" variant="secondary" onClick={() => openModal(customer, 'assign')}>Assegna Punti</Button>
            ...
            {selectedUser && modalView === 'delete' && (<Modal title="Conferma Eliminazione" onClose={closeModal}><div className="dark:text-slate-300"><p className="mb-4">Sei sicuro di voler eliminare l'utente <span className="font-bold">{selectedUser.firstName ? `${selectedUser.firstName} ${selectedUser.lastName}` : selectedUser.username}</span>?<br/>L'username <strong>{selectedUser.username}</strong> tornerà disponibile.</p><div className="flex justify-end gap-3 mt-6"><Button onClick={closeModal} variant="secondary">Annulla</Button><Button onClick={handleDeleteCustomer} variant="danger">Conferma</Button></div></div></Modal>)}
            {showNewCredentials && (<Modal title="Nuovo Cliente Creato" onClose={() => setShowNewCredentials(null)}><div className="dark:text-slate-300"><h3 className="text-xl font-bold mb-4">Credenziali Nuovo Cliente</h3><p className="mb-4">Stampare e consegnare al cliente.</p><div id="new-credentials-printable-content"><div className="bg-slate-100 dark:bg-slate-800 p-4 rounded-lg space-y-2 border dark:border-slate-700"><p><strong>Username:</strong> <span className="font-mono p-1 bg-slate-200 dark:bg-slate-700 rounded">{showNewCredentials.username}</span></p><p><strong>Password:</strong> <span className="font-mono p-1 bg-slate-200 dark:bg-slate-700 rounded">{showNewCredentials.password}</span></p></div></div><div className="flex justify-end gap-3 mt-6"><Button onClick={() => handlePrint('new-credentials-printable-content')} variant="secondary">Stampa</Button><Button onClick={() => setShowNewCredentials(null)}>Chiudi</Button></div></div></Modal>)}
        </Card>
    );
};
