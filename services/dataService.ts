...
export const initData = async (): Promise<void> => {
    ...
    if (adminSnapshot.empty) {
        ...
        const initialActions: Omit<Action, 'id'>[] = [
        ...
        initialActions.forEach(action => {
            ...
            batch.set(actionDocRef, { ...action, id: actionDocRef.id });
        });
...
export const getRecentTransactions = async (count: number = 5): Promise<(PointTransaction & { userName: string })[]> => {
    const q = query(transactionsCollection, orderBy("date", "desc"), limit(count * 2)); 
    const snapshot = await getDocs(q);
    const transactions = snapshot.docs.map(doc => doc.data() as PointTransaction);
...
export const getPrizes = async (): Promise<Prize[]> => {
    ...
    const snapshot = await getDocs(q);
    return snapshot.docs.map(doc => doc.data() as Prize);
};

export const createOrUpdatePrize = async (prize: Omit<Prize, 'id'> & { id?: string }): Promise<Prize> => {
    const docRef = prize.id ? doc(prizesCollection, prize.id) : doc(prizesCollection);
    const newPrize = { ...prize, id: docRef.id };
    await setDoc(docRef, newPrize);
    return newPrize;
};
...
export const createOrUpdateAction = async (action: Omit<Action, 'id'> & { id?: string }): Promise<Action> => {
    const docRef = action.id ? doc(actionsCollection, action.id) : doc(actionsCollection);
    const newAction = { ...action, id: docRef.id };
    await setDoc(docRef, newAction);
    return newAction;
};
